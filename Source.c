...
double totalTime = 19.7171; // from the dataset
...
void DisplayHex(int value)
{
    ...
    *((char*)lowerSeg + 3) = lookupTable[value % 10];
    value /= 10;
    *(upperSeg) = lookupTable[value % 10];
    value /= 10;
    *((char*)upperSeg + 1) = lookupTable[value % 10];
    value /= 10;
    *((char*)upperSeg + 2) = lookupTable[value % 10];
    value /= 10;
    *((char*)upperSeg + 3) = lookupTable[value % 10];
...
void ResetDisplay() {
    ...
    *(upperSeg) = lookupTable[0];
    *((char*)upperSeg + 1) = lookupTable[0];
    *((char*)upperSeg + 2) = lookupTable[0];
    *((char*)upperSeg + 3) = lookupTable[0];
}
...
int ReadButton(void) {
    ...
    volatile int buttonState = *buttonPTR;
...
int main() {
    ...
    int reps = 0, i = 0;
    double assumedMass = 51.35, power = 0, averageAccel = 0; // 51.35 KG
    int length = sizeof(dataset) / sizeof(dataset[0]);

    // Configuration of values
    while (i < length) {

        averageAccel += dataset[i]*0.02;
    ...
    averageAccel /= length;
    power = assumedMass * averageAccel * totalTime * 9.81;
    ...
    while (true)
    {
        if (!ReadButton() && i < length && !ReadSwitch()) {
